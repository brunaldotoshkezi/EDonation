...
"Order canceled"}; // 9
    ...
    public static void KrijoAudit(int orderID, string message,
int messageNumber)
{
// get a configured DbCommand object
DbCommand comm = GenericDataAccess.CreateCommand();
// set the stored procedure name
comm.CommandText = "KrijoAudit";
// create a new parameter
DbParameter param = comm.CreateParameter();
param.ParameterName = "@OrderID";
param.Value = orderID;
...
public class CommerceLibOrderDetailInfo
{
    ...
    public CommerceLibOrderDetailInfo(DataRow orderDetailRow)
    {
        ...
        ProductID = Int32.Parse(orderDetailRow["Produkt_ID"].ToString());
        ProductName = orderDetailRow["Emri"].ToString();
        Quantity = Int32.Parse(orderDetailRow["Sasia"].ToString());
        UnitCost = Double.Parse(orderDetailRow["Cmimi"].ToString());
        // set info property
        Refresh();
    }
...
public class CommerceLibOrderInfo
{
    ...
    public CommerceLibOrderInfo(DataRow orderRow)
    {
        ...
        DateCreated = orderRow["DataKrijuar"].ToString();
        DateShipped = orderRow["DataShipped"].ToString();
        Comments = orderRow["Komente"].ToString();
        ...
        Customer = Membership.GetUser(
          new Guid(orderRow["KlientID"].ToString()));
        CustomerProfile =
